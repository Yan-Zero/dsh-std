...
In a browser-capable profile, the adapter's own DSH browser half reads standard `browser.ui.dsh/v1alpha1 LocalModule` declarations, serves their package-local artifacts, and activates them through DSH's existing client module system and Cordis lifecycle. Components do not need a root Loader entry or `dsh.client` metadata. TUI and headless profiles do not load these modules; all browser peers are optional.
