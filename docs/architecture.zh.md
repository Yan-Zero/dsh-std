...
npm 包版本与协议坐标是两条独立的版本轴。在 `0.1.0-rcN` 发布线上，后续 rc 必须接受该发布线中较早 rc 已经接受的声明、载荷与基础 handler 形状，并保持既有字段与操作的含义。新能力必须是可选字段、可选操作或显式协商的 feature；不能仅因双方声明了同一个 `apiVersion` 就启用。

`0.1.1-rc.N` 发布线开始新的包级兼容周期，可以调整导出的开发 API，但不能原地改写已经发布的协议坐标。领域协议的可观察语义若不兼容，仍必须发布新的 `apiVersion`（例如 `v1alpha2`），并在迁移期按该协议的规则决定是否同时实现旧坐标。
